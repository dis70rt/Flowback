...
func (p *Processor) ProcessTask(ctx context.Context, t *asynq.Task) error {
	...
	if err := json.Unmarshal(t.Payload(), &payload); err != nil {
		slog.Error(fmt.Sprintf("ERROR: Failed to decode task payload: %v\n", err))
		return err
	}

	slog.Info(fmt.Sprintf("CONSUMED: Event=%s\n", payload.Event))
	...
	case events.EventPaymentFailed:
		...
		if err != nil {
			slog.Error(fmt.Sprintf("ERROR: Failed to classify payload: %v\n", err))
			return err
		}

		slog.Info(fmt.Sprintf("-> Payment %s | reason=%q | classification=%s\n",
			result.PaymentID, result.ErrorReason, result.Type))

		switch result.Type {

		case decline.Hard:
			slog.Info(fmt.Sprintf("-> BROADCAST [%s] PaymentID=%s\n", events.TopicDeclineHard, result.PaymentID))
	...
	case events.EventSubscriptionHalted:
		slog.Info(fmt.Sprintf("-> Subscription halted event received (raw=%d bytes)\n", len(payload.RawJSON)))
		// TODO: Trigger dunning flow

	case events.EventSubscriptionPending:
		slog.Info(fmt.Sprintf("-> Subscription pending event received (raw=%d bytes)\n", len(payload.RawJSON)))
		// TODO: Razorpay is retrying

	case events.EventPaymentCaptured:
		slog.Info(fmt.Sprintf("-> Payment captured event received (raw=%d bytes)\n", len(payload.RawJSON)))
		// TODO: Mark invoice paid

	default:
		slog.Info(fmt.Sprintf("-> Unhandled event type=%q (raw=%d bytes) — skipping\n", payload.Event, len(payload.RawJSON)))
	}
